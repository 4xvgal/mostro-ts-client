...
Take a raw BIP-39 seed instead of a mnemonic across `MostroClient`, `Store`, and `restoreSession`; the library no longer receives or persists the mnemonic. Adds `mnemonicToSeed()` and `deriveKeysFromSeed()`.
