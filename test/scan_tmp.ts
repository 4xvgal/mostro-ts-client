import { SimplePool } from "nostr-tools/pool";
import { deriveTradeKeys, unwrapMessageNip44 } from "../src/protocol/index.js";
const MNEMONIC = "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
async function main() {
  const pool = new SimplePool();
  const relay = "ws://localhost:7080";
  const identity = deriveTradeKeys(MNEMONIC, 0);
  const mostro = "6c4b8b42b8bda8e59a155788271ba54febe8bd1dbf1319f3ff0a68d9770ecbbe";
  const evs = await pool.querySync([relay], { kinds: [14], authors: [mostro], "#p": [identity.pubkey] });
  console.log("events:", evs.length);
  for (const e of evs.slice(-6)) {
    try {
      const u = unwrapMessageNip44({ event: { kind: e.kind, pubkey: e.pubkey, content: e.content }, receiverSecretHex: identity.secret });
      console.log(new Date(e.created_at*1000).toISOString(), u?.message.value.action, "req", u?.message.value.request_id);
    } catch { console.log(new Date(e.created_at*1000).toISOString(), "decrypt-fail"); }
  }
  pool.close([relay]);
}
main();
